...
export function UMLChat() {
  ...
  const messages = useMemo(() => {
    ...
    return raw.filter((m) => {
      ...
      // filter out AI messages with empty content (e.g. tool-call-only frames)
  ...
  // update project during streaming (no history recording)
  useEffect(() => {
  ...
  // record one history entry when stream finishes
  useEffect(() => {
  ...
  return (
    <aside className="flex h-full flex-col">
      ...
      <div className="border-b p-3">
        <div className="text-sm font-semibold">UML Copilot Chat</div>
        <div className="text-[11px] text-[#888]">
          Modify tables, fields and relations in natural language
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-auto p-4">
        {messages.length === 0 && (
          <div className="rounded-xl bg-[#f5eef5] p-3 text-xs">
            Import a workbook, then ask: &quot;Rename Customers to Accounts and
            link Orders.customer_id to Accounts.id.&quot;
          </div>
        )}
        ...
        {!!stream.error && (
          <div className="flex gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-700">
            ...
            <div>Agent error: {String(stream.error)}</div>
          </div>
        )}
      </div>
      ...
      <form onSubmit={submit} className="border-t p-3">
        <textarea
          ...
          placeholder="Add, rename or relate tables and fields…"
          onKeyDown={(e) => {
