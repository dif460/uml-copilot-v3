...
export function UMLWorkspace(){
  ...
  const edges=useMemo(()=>p.relations.map(r=>({id:r.id,source:r.sourceTableId,target:r.targetTableId,label:r.type,animated:!!r.inferred,style:r.id===selectedRid?{stroke:"#714B67",strokeWidth:2}:undefined,markerEnd:{type:MarkerType.ArrowClosed}})),[p.relations,selectedRid]);
  ...
  return <section className="relative min-w-0">
    <button onClick={add} className="absolute left-3 top-3 z-20 flex items-center gap-1 rounded bg-[#714B67] px-3 py-2 text-xs text-white"><Plus size={13}/>Add table</button>
