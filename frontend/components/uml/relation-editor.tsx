"use client";import {Trash2,X,ArrowRight} from "lucide-react";import {useUMLStore} from "@/stores/uml-store";import type {RelationType,FieldType} from "@/types/uml-schema";
...
export function RelationEditor(){
  ...
  return <aside className="absolute right-3 top-3 z-20 max-h-[calc(100%-24px)] w-[340px] overflow-auto rounded-xl border bg-white shadow-xl">
    <div className="flex items-center border-b p-3">
      <span className="flex-1 font-semibold text-sm">Relation</span>
    ...
    <div className="p-3 space-y-3">
      ...
      <label className="block text-[10px] text-[#888]">Type</label>
      ...
      <label className="block text-[10px] text-[#888]">Source field ({st?.name})</label>
      ...
      <label className="block text-[10px] text-[#888]">Target field ({tt?.name})</label>
      ...
      <label className="block text-[10px] text-[#888]">Label (optional)</label>
      ...
      <div className="flex gap-2 text-[10px]">
        <label><input type="checkbox" checked={!!r.inferred} onChange={e=>update(r.id,{inferred:e.target.checked})}/> Inferred</label>
      </div>
      <button onClick={()=>{del(r.id);select()}} className="w-full rounded border border-red-200 bg-red-50 p-2 text-xs text-red-700 flex items-center justify-center gap-1">
        <Trash2 size={13}/> Delete relation
      </button>
    </div>
  </aside>
}