...
export const useUMLStore=create<S>((set)=>({project:emptyProject,history:[emptyProject],
 ...
 addTable:()=>set(s=>{const t:UMLTable={id:crypto.randomUUID(),name:"NewTable",fields:[],position:{x:100+s.project.tables.length*30,y:100+s.project.tables.length*30}};const p={...s.project,tables:[...s.project.tables,t],version:s.project.version+1};return{project:p,history:[...s.history,clone(p)]}}),
 ...
 addField:(tid)=>set(s=>{const f:UMLField={id:crypto.randomUUID(),name:"new_field",type:"string",nullable:true};const p={...s.project,tables:s.project.tables.map(t=>t.id===tid?{...t,fields:[...t.fields,f]}:t),version:s.project.version+1};return{project:p,history:[...s.history,clone(p)]}}),
 ...
 addRelation:(r)=>set(s=>{const p={...s.project,relations:[...s.project.relations,{...r,id:crypto.randomUUID()}],version:s.project.version+1};return{project:p,history:[...s.history,clone(p)]}}),
